    ...
    function checkout_maybe_upgrade_schema(): void {
        ...
        try {
            ...
            if ($orderCols) {
                ...
                if (!in_array('secure_delivery', $orderCols, true)) {
                    try {
                        $pdo->exec("ALTER TABLE orders ADD COLUMN secure_delivery TINYINT(1) NOT NULL DEFAULT 0 AFTER delivery_method_details");
                    } catch (Throwable $e) {}
                }
                if (!in_array('secure_delivery_fee', $orderCols, true)) {
                    try {
                        $pdo->exec("ALTER TABLE orders ADD COLUMN secure_delivery_fee DECIMAL(10,2) NOT NULL DEFAULT 0 AFTER secure_delivery");
                    } catch (Throwable $e) {}
                }
            }
        } finally {
...
if (!function_exists('normalize_phone_us_br')) {
    function normalize_phone_us_br(string $phone, string $country = ''): string {
        $raw = trim($phone);
        if ($raw === '') {
            return '';
        }
        ...
        if (strpos($digits, '55') === 0) {
            return ($len === 12 || $len === 13) ? '+'.$digits : '';
        }
        if (strpos($digits, '1') === 0 && $len === 11) {
            return '+'.$digits;
        }
        if ($country === 'BR') {
            return ($len === 10 || $len === 11) ? '+55'.$digits : '';
        }
        if ($country === 'US') {
            return ($len === 10) ? '+1'.$digits : '';
        }
        if ($len === 11) {
...
if (!function_exists('normalize_zipcode_us_br')) {
    function normalize_zipcode_us_br(string $zipcode, string $country = ''): string {
        $digits = preg_replace('/\D+/', '', trim($zipcode));
        if ($digits === '') {
            return '';
        }
        $country = strtoupper(trim($country));
...
if (!function_exists('whatsapp_log_entry')) {
    function whatsapp_log_entry(string $to, string $status, string $detail = ''): void {
        $dir = __DIR__ . '/../storage/logs';
        if (!is_dir($dir)) {
            @mkdir($dir, 0775, true);
        }
        ...
        if ($logLine) {
            @file_put_contents($dir . '/whatsapp.log', $logLine . PHP_EOL, FILE_APPEND);
        }
    }
}
...
if (!function_exists('whatsapp_clean_text')) {
    function whatsapp_clean_text(string $text): string {
        $clean = html_entity_decode($text, ENT_QUOTES, 'UTF-8');
...
if (!function_exists('whatsapp_template_defaults')) {
    function whatsapp_template_defaults(string $storeName): array {
        $storeName = $storeName !== '' ? $storeName : 'Sua Loja';
        return [
            'customer' => 'Pedido {{order_number}} confirmado na '.$storeName.'. Total: {{order_total}}. Obrigado pela compra!',
            'admin' => 'Novo pedido {{order_number}} de {{customer_name}}. Total: {{order_total}}.',
        ];
    }
}

if (!function_exists('twilio_whatsapp_send')) {
    function twilio_whatsapp_send(array $config, string $to, string $body, ?string $contentSid = null, array $contentVars = []): array {
        $sid = trim((string)($config['sid'] ?? ''));
        $token = (string)($config['token'] ?? '');
        $from = whatsapp_normalize_phone((string)($config['from'] ?? ''));
        $toNorm = whatsapp_normalize_phone($to);
        $body = whatsapp_clean_text($body);
        $contentSid = trim((string)$contentSid);
        if ($sid === '' || $token === '' || $from === '' || $toNorm === '' || ($body === '' && $contentSid === '')) {
        ...
        if ($contentSid !== '') {
            $payloadData['ContentSid'] = $contentSid;
            if ($contentVars) {
                $payloadData['ContentVariables'] = json_encode($contentVars, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
            }
        } else {
            $payloadData['Body'] = $body;
        }
        $payload = http_build_query($payloadData, '', '&', PHP_QUERY_RFC3986);
        ...
        if ($response === false) {
            return [false, $status ? 'http_'.$status : 'request_failed'];
        }
        if ($status >= 200 && $status < 300) {
            return [true, 'sent'];
        }
        $detail = $status ? 'http_'.$status : 'http_error';
        $decoded = json_decode((string)$response, true);
        if (is_array($decoded) && !empty($decoded['message'])) {
            $detail .= ': '.$decoded['message'];
        }
...
if (!function_exists('send_order_whatsapp_alerts')) {
    function send_order_whatsapp_alerts(int $order_id, bool $notifyCustomer = true, bool $notifyAdmin = true): bool {
        $enabled = (int)setting_get('whatsapp_twilio_enabled', '0') === 1;
        if (!$enabled) {
            return false;
        }

        $sid = setting_get('whatsapp_twilio_sid', '');
        $token = setting_get('whatsapp_twilio_token', '');
        $fromNumber = setting_get('whatsapp_twilio_from', '');
        $adminTargetsRaw = setting_get('whatsapp_twilio_admin_to', '');
        ...
        $orderNumber = $orderCode !== '' ? $orderCode : '#'.$order_id;
        ...
        if ($trackToken !== '') {
            $trackUrl = $baseUrl ? $baseUrl.'/index.php?route=track&code='.urlencode($trackToken) : '/index.php?route=track&code='.urlencode($trackToken);
        } else {
            $trackUrl = $baseUrl ?: '';
        }

        $defaults = whatsapp_template_defaults($storeName);
        $customerTpl = setting_get('whatsapp_twilio_customer_template', $defaults['customer']);
        $adminTpl = setting_get('whatsapp_twilio_admin_template', $defaults['admin']);
        $customerTemplateSid = '';
        $adminTemplateSid = '';
        if (preg_match('/^HX[0-9a-f]{32}$/i', trim((string)$customerTpl))) {
            $customerTemplateSid = trim((string)$customerTpl);
        }
        if (preg_match('/^HX[0-9a-f]{32}$/i', trim((string)$adminTpl))) {
            $adminTemplateSid = trim((string)$adminTpl);
        }
        ...
        $vars = [
            ...
            'customer_name' => $billingFull !== '' ? $billingFull : 'Cliente',
            'order_total' => format_currency($totalVal, $orderCurrency),
        ];
        $templateVarsCustomer = [
            '1' => $orderNumber,
            '2' => $storeName,
            '3' => format_currency($totalVal, $orderCurrency),
            '4' => $paymentLabel ?: '—',
            '5' => $trackUrl !== '' ? $trackUrl : '—',
        ];
        $templateVarsAdmin = [
            '1' => $orderNumber,
            '2' => $billingFull !== '' ? $billingFull : 'Cliente',
            '3' => format_currency($totalVal, $orderCurrency),
            '4' => $paymentLabel ?: '—',
            '5' => $adminOrderUrl,
        ];
        ...
        if ($notifyCustomer) {
            ...
            if ($customerPhone !== '') {
                if ($customerTemplateSid !== '') {
                    [$sent, $detail] = twilio_whatsapp_send($config, $customerPhone, '', $customerTemplateSid, $templateVarsCustomer);
                } else {
                    $message = email_render_template($customerTpl, $vars);
                    [$sent, $detail] = twilio_whatsapp_send($config, $customerPhone, $message);
                }
                whatsapp_log_entry($customerPhone, $sent ? 'success' : 'failure', $detail);
            } else {
                whatsapp_log_entry('customer', 'skipped', 'missing_phone');
            }
        }

        if ($notifyAdmin) {
            $adminTargets = array_filter(array_map('trim', preg_split('/[,\n;]/', (string)$adminTargetsRaw)));
            if ($adminTargets) {
                foreach ($adminTargets as $adminPhone) {
                    if ($adminTemplateSid !== '') {
                        [$sent, $detail] = twilio_whatsapp_send($config, $adminPhone, '', $adminTemplateSid, $templateVarsAdmin);
                    } else {
                        $message = email_render_template($adminTpl, $vars);
                        [$sent, $detail] = twilio_whatsapp_send($config, $adminPhone, $message);
                    }
                    whatsapp_log_entry($adminPhone, $sent ? 'success' : 'failure', $detail);
                }
            } else {
                whatsapp_log_entry('admin', 'skipped', 'missing_admin_number');
            }
        }
...
if (!function_exists('send_email')) {
    function send_email($to, $subject, $body, $from = null) {
        ...
        if ($useSmtp) {
            ...
            email_log_entry($subject, $to, $smtpResult ? 'success' : 'failure', $smtpDetail, $smtpFromEmail ?: $fromHeader);
            return $smtpResult;
        } else {
            $headers = [
                'From: ' . $fromHeader,
...
if (!function_exists('send_order_admin_alert')) {
    function send_order_admin_alert($order_id, $extraEmails = null) {
        ...
        try {
            ...
            $adminEmail = trim((string)setting_get('admin_email', ''));
            if (!$adminEmail && defined('ADMIN_EMAIL')) {
